...
QString QDateConvertor::GetDayInweek(QDateTime d)
{
    set_days();
    return Day[d.toString("dddd")];
}
...
QStringList QDateConvertor::ToJalali(QString year, QString month,QString day){
   ...
   if (gm>1 && ((gy%4==0 && gy%100!=0) || (gy%400==0)))
    ...
    j_day_no %= 1461;
     ...
     QStringList jalali;
     set_months();
     jalali << QString::number(jy) << ZeroPadding(jm,2)<< ZeroPadding(jd,2)<< Month[jm] ;
     return jalali;
    }
...
QStringList QDateConvertor::ToMiladi(QString year, QString month,QString day ){
    ...
    int Month;
    int Day;
  qDebug()<<"Start Func "<<year<<month<<day;
    ...
    mymiladiDate <<QString::number(miladiDate[0])<<ZeroPadding(miladiDate[1],2)<<ZeroPadding(miladiDate[2],2);
      qDebug()<<mymiladiDate;
    return mymiladiDate;
}
QString QDateConvertor::ZeroPadding(int number,int size)
{
   return  QString("%1").arg(number,size,'d',0,'0');//001;
}
